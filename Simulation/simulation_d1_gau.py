...
'''
GEs_Nchange_mean = np.mean(trails_d1_gau2['GEs_Nchange'], axis=1)
AE_active_m5_Nchange_mean = np.mean(trails_d1_gau2['AE_active_m5_Nchange'], axis=1)
AE_active_m10_Nchange_mean = np.mean(trails_d1_gau2['AE_active_m10_Nchange'], axis=1)
AE_active_m20_Nchange_mean = np.mean(trails_d1_gau2['AE_active_m20_Nchange'], axis=1)
...
ax.set_xlabel('Number of clients ($d$=1)', fontsize='13')
ax.set_ylabel('MSE (Gaussian)', fontsize='13')
plt.yscale('log')
# plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/Increasing_clients_d1.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
# plt.show()
