...
1. load data
'''
loadData = np.load(os.path.dirname(os.getcwd()) + '/Result_data/appendix_d1_boost.npy', allow_pickle=True)
appendix_d1_boost = loadData.tolist()
print(appendix_d1_boost.keys())


# average AE, LE, GE of 20 trails, axis=1 means averaging on 'trail' axis
GEs_mean = [np.mean(appendix_d1_boost['GEs'])] * 20
AEs_mean = np.mean(appendix_d1_boost['AEs'], axis=1)
LEs_mean = np.mean(appendix_d1_boost['LEs'], axis=1)
AE_logs_mean = np.mean(appendix_d1_boost['AE_logs'], axis=1)
AE_log_actives_mean = np.mean(appendix_d1_boost['AE_log_actives'], axis=1)
LE_adapt_opts_mean = np.mean(appendix_d1_boost['LE_adapt_opts'], axis=1)
AE_log_actives_diff_mean = np.mean(appendix_d1_boost['AE_log_actives_diff'], axis=1)
print(GEs_mean)
# 0.0004386382340954275
...
plt.yscale('log')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/boost_d1_paraautonomy.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
...
ax.set_xlabel('Number of local agents ($d$=1)', fontsize='13')
ax.set_ylabel('MSE (boost)', fontsize='13')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/boost_d1_activeeffective.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
...
ax.set_xlabel('Number of local agents ($d$=1)', fontsize='13')
ax.set_ylabel('MSE (boost)', fontsize='13')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/boost_d1_same_diff_compare.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
