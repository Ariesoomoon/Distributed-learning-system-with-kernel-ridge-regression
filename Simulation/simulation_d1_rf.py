...
GEs_mean = [np.mean(appendix_d1_rf['GEs'])] * 20
AEs_mean = np.mean(appendix_d1_rf['AEs'], axis=1)  # average AE, LE, GE of 20 trails, axis=1 means averaging on trail axis
LEs_mean = np.mean(appendix_d1_rf['LEs'], axis=1)
AE_logs_mean = np.mean(appendix_d1_rf['AE_logs'], axis=1)
AE_log_actives_mean = np.mean(appendix_d1_rf['AE_log_actives'], axis=1)
LE_adapt_opts_mean = np.mean(appendix_d1_rf['LE_adapt_opts'], axis=1)
AE_log_actives_diff_mean = np.mean(appendix_d1_rf['AE_log_actives_diff'], axis=1)
...
plt.yscale('log')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/rf_d1_paraautonomy.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
...
ax.set_xlabel('Number of local agents ($d$=1)', fontsize='13')
ax.set_ylabel('MSE (rf)', fontsize='13')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/rf_d1_activeeffective.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
...
ax.set_xlabel('Number of local agents ($d$=1)', fontsize='13')
ax.set_ylabel('MSE (rf)', fontsize='13')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/rf_d1_same_diff_compare.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
