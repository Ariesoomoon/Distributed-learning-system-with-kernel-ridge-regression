...
GEs_mean = [np.mean(appendix_d5_krr['GEs'])] * 20
AEs_mean = np.mean(appendix_d5_krr['AEs'], axis=1)  # average AE, LE, GE of 20 trails, axis=1 means averaging on trail axis
LEs_mean = np.mean(appendix_d5_krr['LEs'], axis=1)
AE_logs_mean = np.mean(appendix_d5_krr['AE_logs'], axis=1)
AE_log_actives_mean = np.mean(appendix_d5_krr['AE_log_actives'], axis=1)
LE_adapt_opts_mean = np.mean(appendix_d5_krr['LE_adapt_opts'], axis=1)
AE_log_actives_diff_mean = np.mean(appendix_d5_krr['AE_log_actives_diff'], axis=1)
print(GEs_mean)
...
plt.yscale('log')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/krr_d5_paraautonomy.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
...
ax.set_xlabel('Number of local agents ($d$=5)', fontsize='13')
ax.set_ylabel('MSE (Krr)', fontsize='13')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/krr_d5_activeeffective.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
print(AE_logs_mean)
...
ax.set_xlabel('Number of local agents ($d$=5)', fontsize='13')
ax.set_ylabel('MSE (Krr)', fontsize='13')
plt.savefig(os.path.dirname(os.getcwd()) + '/Result_figure/krr_d5_same_diff_compare.pdf', dpi=600, format='pdf', bbox_inches='tight', pad_inches=0.2)
plt.show()
