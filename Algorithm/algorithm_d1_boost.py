...
train, test, d = 2000, 1000, 1  # training size; testing size; dimensional of data
...
g_ts = appendix_d1_boost['g_t']
l_ts = appendix_d1_boost['l_t']
l_ts_diff = appendix_d1_boost['l_t_diff']
...
for th in range(trails):
    ...
    g_t = g_ts[th]    # for th trail, select the corresponding g_ts[th]
    l_t = l_ts[th]
...
3. save MSE
'''
appendix_d1_boost['GEs'] = GEs
appendix_d1_boost['LEs'] = LEs                      # each local agent uses the global optimal localization parameter
appendix_d1_boost['AEs'] = AEs
appendix_d1_boost['AE_logs'] = AE_log
appendix_d1_boost['AE_log_actives'] = AE_log_actives

appendix_d1_boost['LE_adapt_opts'] = LE_adapt_opts  # each local agent uses its own optimal localization parameter
appendix_d1_boost['AE_log_actives_diff'] = AE_log_actives_diff


np.save(os.path.dirname(os.getcwd()) + '/Result_data/appendix_d1_boost.npy', appendix_d1_boost)
print('--------------------------------------------------------------save appendix_d1_boost.npy done')
print(appendix_d1_boost.keys())
